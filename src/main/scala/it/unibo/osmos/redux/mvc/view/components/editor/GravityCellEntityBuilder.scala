...
/**
  ...
  */
class GravityCellEntityBuilder(val isAttractive: Boolean) extends CellEntityBuilder {

  /* Specific weight node*/
  val weight: DoubleProperty = DoubleProperty(0.0)
  val weightNode = new VBox(2.0, new TitledDoubleField("Weight: ", weight).innerNode)
