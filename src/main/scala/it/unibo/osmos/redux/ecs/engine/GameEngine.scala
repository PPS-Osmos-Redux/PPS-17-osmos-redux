...
object GameEngine {
  ...
  private case class GameEngineImpl(private val framerate: Int = 30) extends GameEngine {
    ...
    override def init(level: Level, levelContext: LevelContext): Unit = {
      ...
      systems ++= List(MovementSystem(), CollisionSystem(), SpawnSystem(), DrawSystem(levelContext), CellsEliminationSystem(), EndGameSystem(levelContext, level.isSimulation, level.victoryRule))
