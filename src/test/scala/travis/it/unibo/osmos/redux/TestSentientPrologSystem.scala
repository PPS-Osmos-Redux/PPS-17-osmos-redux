...
class TestSentientPrologSystem extends FunSuite with BeforeAndAfter {
  ...
  test("Sentient cell hunting smaller cells") {
    ...
    val sentientCellEntity = SentientCellBuilder().withPosition(scp).withDimension(scd).build

    val scd2 = DimensionComponent(8)
    val scp2 = PositionComponent(Point(101, 87))
    val sentientCellEntity2 =  SentientCellBuilder().withPosition(scp2).withDimension(scd2).build
    ...
    val cellEntity1 = new CellBuilder().withAcceleration(ca1).withDimension(cd1).withPosition(cp1).withSpeed(cs1).build

    val ca2 = AccelerationComponent(1, 1)
    val cd2 = DimensionComponent(8)
    val cp2 = PositionComponent(Point(30, 100))
    val cellEntity2 = new CellBuilder().withAcceleration(ca2).withDimension(cd2).withPosition(cp2).build
    ...
    val cellEntity3 = new CellBuilder().withAcceleration(ca3).withDimension(cd3).withPosition(cp3).withSpeed(cs3).build
